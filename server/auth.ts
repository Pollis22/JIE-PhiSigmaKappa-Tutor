...
export function setupAuth(app: Express) {
  ...
  // Production: secure=true, sameSite='lax', domain='.phisig-tutor.ai' for iOS Safari compatibility
  ...
  // - Override: Set SESSION_COOKIE_DOMAIN=".phisig-tutor.ai" if cross-subdomain cookies needed
  // 
  ...
  // - Canonical redirect to www.phisig-tutor.ai (ensures consistent host)
  ...
  app.post("/api/auth/trial-signup", async (req, res, next) => {
    try {
      ...
      const leadEmail = process.env.JIE_LEAD_NOTIFY_EMAIL || process.env.ADMIN_EMAIL || 'leads@phisig-tutor.ai';
