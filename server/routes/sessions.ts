...
router.get('/:id/export', async (req, res) => {
  try {
    ...
    if (format === 'pdf') {
      ...
      doc.fontSize(24).font('Helvetica-Bold').text('Phi Sigma Kappa Tutor', { align: 'center' });
      ...
      doc.fontSize(7).fillColor('#999999').text(
        `Generated by Phi Sigma Kappa Tutor | ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`,
        { align: 'center' }
      );
