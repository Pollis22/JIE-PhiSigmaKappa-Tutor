...
function evaluateObservationFlags(obs: any): ObservationFlag[] {
  ...
  if (obs.avg_engagement_score < 1.5 && sessions >= 7) {
    const rawObs = `${name}'s average engagement score across ${sessions} sessions is ${obs.avg_engagement_score.toFixed(1)} out of 5. This score reflects how much ${name} elaborates in responses relative to tutor prompts within Phi Sigma Kappa Tutor sessions.`;
