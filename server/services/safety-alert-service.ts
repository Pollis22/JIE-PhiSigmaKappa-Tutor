...
export async function sendAdminSafetyAlert(data: SafetyAlertData): Promise<boolean> {
  ...
  const subject = `[Phi Sigma Kappa Tutor ALERT] ${data.flagType} - Session ${data.sessionId.slice(0, 8)}`;
...
This is an automated alert from Phi Sigma Kappa Tutor Safety System.
  `.trim();
...
export async function sendParentAlert(data: SafetyAlertData): Promise<boolean> {
  ...
  const subject = `[Phi Sigma Kappa Tutor] Tutoring Session Alert - ${data.studentName || 'Your Child'}`;
...
export async function sendJIESupportNotification(data: SafetyIncidentNotification): Promise<boolean> {
  ...
  const subject = isCritical 
    ? `[Phi Sigma Kappa Tutor CRITICAL] ${incidentLabel} - Immediate Review Required`
    : `[Phi Sigma Kappa Tutor SAFETY] ${incidentLabel} - Session ${data.sessionId.slice(0, 8)}`;
