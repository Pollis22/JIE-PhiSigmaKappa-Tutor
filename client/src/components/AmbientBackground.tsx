...
const CAMPUS_PHRASES = ['Brotherhood', 'Scholarship', 'ΦΣΚ', '1873'];
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < CONNECTION_DIST) {
            ...
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.strokeStyle = `rgba(183, 18, 52, ${alpha})`;
            ctx.lineWidth = 0.8;
            ctx.stroke();
          }
        }
      ...
      nodes.forEach(n => {
        ...
        if (n.isCampus) {
          ...
          ctx.fillStyle = `rgba(183, 18, 52, ${alpha * 0.85})`;
          ctx.fillText(n.symbol!, n.x, n.y);
        } else if (n.symbol) {
          ctx.font = `${10 + n.r * 2}px serif`;
          ctx.fillStyle = `rgba(183, 18, 52, ${alpha * 0.75})`;
          ctx.fillText(n.symbol, n.x, n.y);
        } else {
          ctx.beginPath();
          ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(183, 18, 52, ${alpha})`;
          ctx.fill();
        }
      });
